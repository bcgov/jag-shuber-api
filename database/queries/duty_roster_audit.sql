...
FROM    :pg_app_schema.aud_sheriff_duty ausd
...
FROM    :pg_app_schema.sheriff_duty sd
...
FROM    :pg_app_schema.aud_sheriff_duty ausd_op
JOIN    :pg_app_schema.aud_sheriff_duty ausd_data ON ausd_data.sheriff_duty_id = ausd_op.sheriff_duty_id AND ausd_data.revision_count = ausd_op.revision_count + 1
...
FROM    :pg_app_schema.aud_sheriff_duty ausd
JOIN    :pg_app_schema.sheriff_duty sd ON sd.sheriff_duty_id = ausd.sheriff_duty_id AND sd.revision_count = ausd.revision_count + 1
...
LEFT OUTER JOIN    :pg_app_schema.duty dty ON dty.duty_id = aud.duty_id
LEFT OUTER JOIN    :pg_app_schema.assignment asn on asn.assignment_id = dty.assignment_id
LEFT OUTER JOIN    :pg_app_schema.sheriff shr ON shr.sheriff_id = aud.sheriff_id
